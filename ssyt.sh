...
optimize_memory() {
    ...
    if [ -w /proc/sys/vm/swappiness ]; then
        ...
        echo -e "${YELLOW}警告：无法修改vm.swappiness，需要root权限${NC}"
    fi
    
    if [ -w /proc/sys/vm/vfs_cache_pressure ]; then
        echo "50" > /proc/sys/vm/vfs_cache_pressure
    else
        echo -e "${YELLOW}警告：无法修改vm.vfs_cache_pressure，需要root权限${NC}"
    fi
    
    # Alpine特有优化
    if rc-update del local default 2>/dev/null; then
        echo -e "${GREEN}已禁用不必要的本地服务${NC}"
    else
        echo -e "${YELLOW}警告：无法禁用本地服务${NC}"
    fi
}
